...
async function dmToFollowers() {
    ...
    try {
        ...
        var target = login.username

        log(chalk.bold.green(`Logged in as @${login.username} (User ID: ${login.pk})`));
        log(chalk.bold.yellow(`Collecting follower information of @${target} . . .`));

        var id   = await ig.getIdByUsername(login.username)
